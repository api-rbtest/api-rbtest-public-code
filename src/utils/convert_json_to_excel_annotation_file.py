...
def convert_json_to_excel_request_response_constraints(json_file, openapi_spec_file, output_file):
    if not os.path.exists(json_file) or not os.path.exists(openapi_spec_file):
        ...
        return
    openapi_spec = json.load(open(openapi_spec_file, "r"))
...
def main():
    service_names = ["Canada Holidays", "GitLab Project", "GitLab Repository", "GitLab Commit", "GitLab Groups", "GitLab Issues",  "GitLab Branch", "StripeClone", ]
    output_folder = "experiment_1"
    experiment_folder = "evaluation/experiment_1"
    approach = ""
    for service_name in service_names:
        convert_json_to_excel_request_response_constraints(service_name, experiment_folder, "request_response_constraints.xlsx", output_folder)
        convert_json_to_excel_response_property_constraints(service_name, experiment_folder, "response_property_constraints.xlsx", output_folder)
